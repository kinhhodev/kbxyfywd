...
- `src/hook/wpe_hook.cpp` and `include/hook/wpe_hook.h`: hook lifecycle, packet interception, response dispatch, automation logic.
...
- `src/app/` holds the desktop entrypoint and top-level window/bootstrap logic.
- `src/hook/` holds the socket hook and automation implementation. Even after the folder split, much of the activity implementation is still centralized in `src/hook/wpe_hook.cpp`; prefer extending that file for minimal diffs unless the task is an intentional refactor.
- `src/protocol/` holds packet parsing/building implementations.
- `src/core/` holds shared native helpers such as the WebView bridge, web message handler, interceptor, and utility code.
